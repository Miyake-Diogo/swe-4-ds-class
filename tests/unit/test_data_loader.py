...
from src.data_loader import get_feature_names, load_credit_data
...
class TestLoadCreditData:
    ...
    def test_loads_from_cache_when_exists(self, mocker, sample_credit_data):
        ...
        mock_read_parquet = mocker.patch(
            "src.data_loader.pd.read_parquet", return_value=sample_credit_data
        )
    ...
    def test_loads_from_csv_when_no_cache(self, mocker, sample_credit_data, tmp_path):
        """
    ...
    def test_raises_filenotfound_for_missing_file(self, mocker):
        ...
        with pytest.raises(FileNotFoundError) as exc_info:
            load_credit_data(filepath=Path("nao_existe.csv"), use_cache=False)
    ...
    def test_raises_valueerror_for_empty_csv(self, mocker, tmp_path):
        ...
        mocker.patch.object(Path, "exists", lambda self: str(self) == str(empty_csv))
